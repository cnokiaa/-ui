...
'touch'
	...
	var Record = Class.create({
		...
		initialize : function() {
			...
			this.scrollBack();
			// 查看详情
			$("#scroller").on('tap', "ul li", function() {
				var a = $(this).attr("data-orderid");
				var b = $(this).attr("data-period");
				window.location.href = "hdw/application/query/applyLoanDetails?contractno=" + a + "&period=" + b;
			});
		},
		// 初始事件
		startEvent : function() {
		...
		AJAX : function(successCB, errorCB) {
			...
			$.ajax({
				type : "GET",
				// url : "../json/test.json",
				url : "hdw/application//query/applyLoansRecord",
				dataType : "json",
		...
		getStr : function(arr) {
			...
			for(var i = 0; i < arr.length; i++) {
				...
				str += '<li data-period="' + dataPart.period + '" data-orderid="' + dataPart.contractno + '" class="' + sta.typeClass + '">'+
								'<div class="list-left">'+
