...
'touch'
	...
	var Trial = Class.create({
		...
		getChoseList : function() {
			...
			$.ajax({
				type : "post",
				url : "hdw/application/query/product",
		...
		ajaxDate : function(productno) {
			this.BeforeAjaxPublic(function(applyamt) {
				...
				$.ajax({
					type : "POST",
					url : "hdw/application/query/plan",
		...
		subDate : function() {
			...
			$(".next-step").on("tap", function() {
				...
				if(muchM == "") {
					...
					if(applyamt.toString() == "NaN") {
						_this.popup("请输入有效的数字");
					} else {
						// 是数字，判断输入范围
						if(applyamt < _this.limitMin || applyamt > _this.limitMax) {
							_this.popup("最低" + _this.limitMin + "元，最高" + _this.limitMax + "元");
						} else {
							// 判断是否选择类型
