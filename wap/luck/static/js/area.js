...
function change(v){console.log(v);
    var str="0";
    /*for(i=0;i<v;i++){
        str+=("_"+($(s[v]).index()+1));
    };*/
    var ss=document.getElementById("s_province");
    with(ss){
        length = 0;
        //options[0]=new Option(opt0[v],opt0[v]);
       // console.log($(s[v-1]).index()+1);
        if(v && $(s[v-1]).index()>-2 || !v){
            console.log("####"+str);
            if(dsy.Exists(str)){
                ar = dsy.Items[str];
                var dhtml = "";
...
function changeP(v){
    ...
    if(dsy.Exists(str)){
        ...
        for(i=0;i<ar.length;i++){
           // options[length]=new Option(ar[i],ar[i]);
            dhtml +='<a data-id="0_'+v+'_'+i+'">'+ar[i]+'</a>'; 
            console.log(ar[i]);
        }//end for
        //console.log(dhtml);
        $("#s_city").html(dhtml);
...
function changePe(v){console.log(v);
    var str = v;
    if(dsy.Exists(str)){
        ar = dsy.Items[str];
        var dhtml = "";
...
function _init_area(){  //初始化函数
    /*for(i=0;i<s.length-1;i++){console.log("afafafafafa"+i);
        $("#"+s[i]).delegate("a","click",function(){
            changeP($(this).attr("data-id"),i);
           $(".selecta").hide();
          //  $("."+s[i+1]).show();
        });
        //document.getElementById(s[i]).getElementsByTagName("a").onclick=new Function("change("+(i+1)+",this)");
    }*/
    change(0);
}
var areaprovince ="",areacity = "",areacounty = "";
$("#"+s[0]).delegate("a","click",function(){
    changeP($(this).attr("data-id"));
   // $("."+s[0]).hide();
    $(".selecta").hide();
    $("."+s[1]).show();
    areaprovince = $(this).html();
});
$("#"+s[1]).delegate("a","click",function(){
    changePe($(this).attr("data-id"));
   // $("."+s[0]).hide();
   $(".selecta").hide();
    $("."+s[2]).show();
    areacity = $(this).html();
});
$("#"+s[2]).delegate("a","click",function(){
    areacounty = $(this).html();
    $(".selectarea").val(areaprovince+areacity+areacounty);
    $(".selecta,.conv").hide();
});
$(".selecta .qx").on("click",function(){
    $(".selecta").hide();
    if($(this).hasClass("active")){
        $(".conv").hide();
    } else {
        $(this).parents(".selecta").prev().show();
    }
});